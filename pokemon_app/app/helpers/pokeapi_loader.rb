class PokeapiLoader
  ...
  POKEMON_SERVICE_PROBE = "#{POKEMON_SERVICE}?limit=1"
  POKEMON_SERVICE_FETCH_LIMIT = 100
  POKEMON_SERVICE_FETCH = "#{POKEMON_SERVICE}?limit=#{POKEMON_SERVICE_FETCH_LIMIT}&offset="
  ...
  def add_abilities(abilities, db_pokemon)
    # if there are abilities, process and fill
    if abilities.size > 0
  ...
  def add_forms(forms, db_pokemon)
    # if there are forms, process and fill
    if forms.size > 0
  ...
  def add_held_items(held_items, db_pokemon)
    # if there are items, process and fill
  ...
  def add_moves(moves, db_pokemon)
    # if there are moves, process and fill
    if moves.size > 0
  ...
  def add_stats(stats, db_pokemon)
    # if there are stats, process and fill
    if stats.size > 0
