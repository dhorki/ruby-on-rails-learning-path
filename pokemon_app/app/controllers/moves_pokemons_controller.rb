...
class MovesPokemonsController < ApplicationController
  ...
  def create
    ...
    if @moves_pokemon.save
      flash[:notice] =
        "The Pokemon Move '#{@moves_pokemon.move.name} (id: #{@moves_pokemon.move.id})' "\
        "for '#{@moves_pokemon.pokemon.name} (id: #{@moves_pokemon.pokemon.id})' was added successfully"
      redirect_to(moves_pokemons_path(@moves_pokemon, @nav_params))
    else
      render('new')
    end
  end
  ...
  def update
    ...
    if @moves_pokemon.update(moves_pokemon_params)
      ...
      redirect_to(moves_pokemon_path(@moves_pokemon, @nav_params))
    else
  ...
  def destroy
    ...
    redirect_to(moves_pokemons_path(@nav_params))
  end
  ...
  def move_page
    @move_page = params[:page] || params[:move_page]
  end
