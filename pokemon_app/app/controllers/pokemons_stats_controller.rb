...
class PokemonsStatsController < ApplicationController
  ...
  def create
    ...
    if @pokemon_stat.save
      flash[:notice] =
        "The Pokemon Stat '#{@pokemon_stat.stat.name} (id: #{@pokemon_stat.stat.id})' "\
        "for '#{@pokemon_stat.pokemon.name} (id: #{@pokemon_stat.pokemon.id})' was added successfully"
      redirect_to(pokemons_stat_path(@pokemon_stat, @nav_params))
    else
      render('new')
    end
  end
  ...
  def destroy
    ...
    redirect_to(pokemons_stats_path(@nav_params))
  end
  ...
  def stat_page
    @stat_page = params[:page] || params[:stat_page]
  end
