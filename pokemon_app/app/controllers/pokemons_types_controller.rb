...
class PokemonsTypesController < ApplicationController
  ...
  def create
    ...
    if @pokemon_type.save
      flash[:notice] =
        "The Pokemon Type '#{@pokemon_type.type.name} (id: #{@pokemon_type.type.id})' "\
        "for '#{@pokemon_type.pokemon.name} (id: #{@pokemon_type.pokemon.id})' was added successfully"
      redirect_to(pokemons_type_path(@pokemon_type, @nav_params))
    else
      render('new')
    end
  end
  ...
  def destroy
    ...
    redirect_to(pokemons_types_path(@nav_params))
  end
  ...
  def type_page
    @type_page = params[:page] || params[:type_page]
  end
